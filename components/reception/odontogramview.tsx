...
export function OdontogramView({
  ...
  const latestToothStates: Record<
    string,
  ...
  const renderDentalGroup = (teeth: number[]) => (
    <div className="flex gap-1.5 bg-white p-2 rounded-xl border border-slate-100 shadow-sm flex-1 justify-center overflow-x-auto min-w-[200px]">
      {teeth.map((toothNum) => {
        const toothInfo = latestToothStates[toothNum.toString()] || {
          status: "Healthy",
          notes: "",
          problems: [],
        };
        ...
        return (
          ...
          >
            ...
            <svg
              viewBox="0 0 24 24"
              className={`w-5 h-5 transition-colors duration-200 ${statusStyles.icon}`}
            >
              <path
                fill="currentColor"
                fillOpacity="0.08"
            ...
            <span
              className={`mt-1 text-[7px] font-black uppercase px-0.5 rounded shadow-sm border ${statusStyles.labelBg} border-slate-200/20`}
            >
      ...
      <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-6">
        <div>
          <h3 className="text-sm font-bold text-slate-800 uppercase tracking-wider flex items-center gap-2 mb-1">
            <Activity className="w-4 h-4 text-brand-600" /> Patient Odontogram
            Chart
          </h3>
          <p className="text-xs text-slate-500 font-medium">
            Click any tooth below to review its complete historical transition
            logs, clinical comments, and diagnostic problem tags.
          </p>
        </div>
        ...
        <div className="flex flex-wrap gap-3 items-center justify-center p-3 bg-slate-50/50 border border-slate-100 rounded-2xl">
          {[
            ...
            >
              <span
                className={`w-2.5 h-2.5 rounded-full ${legendItem.color} shadow-sm shrink-0`}
              ></span>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden min-h-[250px] flex flex-col">
        <div className="p-4 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center shrink-0">
          <h3 className="text-xs font-bold text-slate-800 uppercase tracking-wider flex items-center gap-2">
            ...
            {selectedTooth
              ? `Tooth #${selectedTooth} Diagnosis Timeline`
              : "Patient Tooth History Summary"}
          </h3>
          {selectedTooth && (
            <button
                ...
                if (history.length === 0) {
                  return (
                    <div className="text-center py-12 text-slate-400 italic text-xs">
                      No clinical problems or interventions logged for Tooth #
                      {selectedTooth}. It is healthy.
                    </div>
                  );
                }
                return (
                  <div className="relative border-l-2 border-slate-100 pl-6 ml-4 space-y-6">
                    {history.map((record, index) => {
                      ...
                      return (
                        ...
                        >
                          <span
                            className={`absolute -left-[30px] top-1.5 flex h-4 w-4 items-center justify-center rounded-full border-2 border-white ${statusStyles.icon} bg-white shadow-sm`}
                          >
                            <span
                              className={`h-2 w-2 rounded-full ${statusStyles.icon.replace("text-", "bg-")}`}
                            ></span>
                          </span>
                          <div className="space-y-1.5">
                            <div className="flex flex-wrap items-center gap-3">
                              ...
                              <span
                                className={`text-[9px] font-black uppercase px-1.5 py-0.5 rounded shadow-sm border ${statusStyles.labelBg} border-slate-200/20`}
                              >
                                {record.status}
                              </span>
                            </div>
                            ...
                            {record.problems && record.problems.length > 0 && (
                              <div className="flex flex-wrap gap-1">
                                {record.problems.map((prob) => (
                                  <span
                                    key={prob}
                                    className="px-2 py-0.5 bg-red-50 text-red-700 border border-red-100/50 rounded-md text-[9px] font-bold"
                                  >
                                    {prob}
                                  </span>
                                ))}
            ...
            <div className="flex-1 flex flex-col justify-start">
              {hasToothProblems ? (
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {Object.keys(toothHistoryMap)
                      ...
                      .map((tNum) => {
                        ...
                        const latest = latestToothStates[tNum] || {
                          status: "Healthy",
                          notes: "",
                          problems: [],
                        };
                          ...
                          >
                            <div className="flex items-center gap-3">
                              ...
                              <div className="min-w-0">
                                <span
                                  className={`text-[8px] font-black uppercase px-1.5 py-0.5 rounded border ${statusStyles.labelBg} border-slate-200/20 inline-block mb-1`}
                                >
                                  {latest.status}
                                </span>
                ...
                <div className="text-center py-12 text-slate-400 italic text-xs my-auto">
                  No previous dental anomalies, pathologies or treatments are
                  registered in the system for this patient.
                </div>
              )}
            </div>
          )}
