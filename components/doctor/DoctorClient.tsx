            ...
            filteredPatients.map(p => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-black text-slate-900 text-sm truncate">{p.firstName} {p.lastName}</p>
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-8 lg:p-12">
        {selectedPatient ? (
          <div className="max-w-4xl mx-auto space-y-8">
            {/* iPad Header */}
            <div className="bg-white p-6 md:p-8 rounded-[2rem] border border-slate-200 shadow-xl flex flex-col md:flex-row justify-between items-center gap-6">
               <div className="flex gap-6 items-center w-full md:w-auto">
                  <div className="w-16 h-16 md:w-20 md:h-20 bg-indigo-600 text-white rounded-3xl flex items-center justify-center shadow-lg transform -rotate-3">
                     ...
                     <form action={async (formData) => {
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                           <div className="space-y-2">
                              <label className="text-xs font-black text-slate-500 uppercase tracking-widest ml-1">Past Medical History</label>
                              <textarea
                                name="pastHistory"
                                rows={6}
                                defaultValue={selectedPatient.diagnosis?.pastHistory || ""}
                                className="w-full p-5 bg-slate-50 border-2 border-slate-100 rounded-[1.5rem] outline-none focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all text-slate-800 font-medium resize-none text-lg"
                                placeholder="Describe past surgeries, conditions..."
                              />
                           </div>
                           <div className="space-y-2">
                              <label className="text-xs font-black text-slate-500 uppercase tracking-widest ml-1">Active Medical Conditions (comma separated)</label>
                              <textarea
                                name="medicalHistory"
                                rows={6}
                        ...
                        <div className="flex justify-end pt-4">
                           <button type="submit" className="w-full md:w-auto bg-indigo-600 text-white px-12 py-5 rounded-2xl font-black text-lg hover:bg-indigo-700 shadow-xl shadow-indigo-200 transition-all flex items-center justify-center gap-3 active:scale-95">
                     ...
                     <form action={async (formData) => {
                        ...
                        <div className="space-y-6">
                           <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                              <div className="space-y-2">
                                 <label className="text-xs font-black text-slate-500 uppercase tracking-widest ml-1">Visit Title / Purpose</label>
                                 <input name="title" defaultValue={selectedPatient.medicalRecord?.title || ""} className="w-full p-5 bg-slate-50 border-2 border-slate-100 rounded-2xl outline-none focus:border-indigo-500 transition-all text-lg font-bold" placeholder="e.g. Toothache, Scaling..." />
                              </div>
                              <div className="space-y-2">
                                 <label className="text-xs font-black text-slate-500 uppercase tracking-widest ml-1">Status</label>
                                 <select name="status" defaultValue={selectedPatient.medicalRecord?.status || "STABLE"} className="w-full p-5 bg-slate-50 border-2 border-slate-100 rounded-2xl outline-none focus:border-indigo-500 transition-all text-lg font-bold appearance-none bg-white">
                           ...
                           <div className="space-y-2">
                              <label className="text-xs font-black text-slate-500 uppercase tracking-widest ml-1">Current Complaints & Examination Findings</label>
                              <textarea
                                name="complaints"
                                rows={8}
                                defaultValue={selectedPatient.medicalRecord?.complaints || ""}
                                className="w-full p-5 bg-slate-50 border-2 border-slate-100 rounded-[1.5rem] outline-none focus:border-indigo-500 transition-all text-slate-800 font-medium text-lg"
                                placeholder="Patient reports pain in upper right molar..."
                              />
                           </div>
                        </div>
                        <div className="flex justify-between items-center pt-4 gap-4">
                           <button type="button" onClick={() => setCurrentStep(1)} className="flex-1 md:flex-none bg-slate-100 text-slate-700 px-8 py-5 rounded-2xl font-black text-lg hover:bg-slate-200 transition-all flex items-center justify-center gap-2">
                              <ChevronLeft className="w-6 h-6" /> Back
                           </button>
                           <button type="submit" className="flex-1 md:flex-none bg-indigo-600 text-white px-12 py-5 rounded-2xl font-black text-lg hover:bg-indigo-700 shadow-xl shadow-indigo-200 transition-all flex items-center justify-center gap-3 active:scale-95">
                     ...
                     <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-500">
                        <div className="space-y-6">
                           {newProcedures.map((proc, index) => (
                              <div key={index} className="p-8 border-2 border-slate-100 rounded-[2rem] relative bg-slate-50/50 space-y-6 shadow-sm">
                                 {newProcedures.length > 1 && (
                                    <button onClick={() => removeProcedureRow(index)} className="absolute -top-4 -right-4 bg-white text-red-500 p-2.5 rounded-2xl shadow-xl hover:bg-red-50 transition-all border border-red-100"><Trash2 className="w-6 h-6" /></button>
                                 )}
                                 <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                    <div className="md:col-span-2 space-y-1">
                                       <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Procedure</label>
                                 ...
                                 <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div className="space-y-1">
                                       <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-1"><Pill className="w-3 h-3 text-red-500" /> Medicines</label>
                                    ...
                                    <div className="space-y-1">
                                       <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-1"><Lightbulb className="w-3 h-3 text-yellow-500" /> Suggestions</label>
                           ...
                           <button onClick={addProcedureRow} className="w-full py-8 border-4 border-dashed border-slate-100 rounded-[2rem] text-slate-400 font-black text-lg hover:bg-slate-50 hover:border-indigo-200 hover:text-indigo-500 transition-all flex items-center justify-center gap-3">
                        ...
                        <div className="flex justify-between items-center pt-8 gap-4 border-t border-slate-100">
                           <button onClick={() => setCurrentStep(2)} className="flex-1 md:flex-none bg-slate-100 text-slate-700 px-8 py-5 rounded-2xl font-black text-lg hover:bg-slate-200 transition-all flex items-center justify-center gap-2">
                              <ChevronLeft className="w-6 h-6" /> Back
                           </button>
                           <button
                             onClick={async () => {
                           ...
                           >
                              Save & Finalize Session <CheckCircle2 className="w-6 h-6" />
                           </button>
                        </div>
                     </div>
                  )}
            ...
            {/* History Preview (Fixed at bottom for iPad convenience) */}
            <div className="bg-white p-6 rounded-[2rem] border border-slate-200 shadow-sm">
                ...
                <div className="flex gap-4 overflow-x-auto pb-2">
                   {selectedPatient.procedures?.map((proc, i) => (
                      <div key={i} className="min-w-[200px] p-4 bg-slate-50 rounded-2xl border border-slate-100 shrink-0">
                         <p className="text-[10px] font-black text-indigo-600 uppercase">{new Date(proc.procedureDate).toLocaleDateString()}</p>
          ...
          <div className="h-full flex flex-col items-center justify-center text-slate-300 space-y-6 animate-in fade-in zoom-in-95 duration-700">
            <div className="bg-white p-12 rounded-[3rem] shadow-2xl border border-slate-100 flex flex-col items-center">
               ...
               <p className="font-black text-2xl text-slate-400">Select a patient to begin</p>
