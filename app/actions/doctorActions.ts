...
export async function updateDiagnosis(patientId: string, formData: FormData) {
  ...
  if (finalize) {
    ...
    // Create follow-up appointment and GENERATE BILL
    if (nextVisitDate && !isNaN(nextVisitDate.getTime())) {
      ...
      // FETCH DEFAULT APPOINTMENT FEE
      const settings = await prisma.systemSettings.findUnique({
        where: { id: "default" },
      });
      const defaultFee = settings?.appointmentFee || 0;
      ...
      // 2. Automatically generate the Follow-up Bill (Procedure)
      // This will instantly show up in the Receptionist's Billing Dashboard
      await prisma.procedure.create({
        data: {
          id: `followup-bill-${followUpAppt.id}`, // Link it logically to the appointment
          patientId,
          appointmentId: followUpAppt.id, // Explicit relation if defined in schema
          name: "Follow-up Consultation",
          type: "Appointment",
          cost: defaultFee, // Apply the default fee set by admin
          status: "PENDING",
          procedureDate: nextVisitDate, // Bill is dated for the actual follow-up day
          description: "System generated bill for scheduled follow-up visit.",
        },
      });

      // 3. Update patient's primary doctor assignment
